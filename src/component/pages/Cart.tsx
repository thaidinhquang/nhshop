            ...
            <div className="container md:flex md:justify-between gap-8 py-12">
                {isLoading || isLoadingProduct ? <div>Loading...</div> :
                    <table className='table-fixed w-full'>
                        ...
                        <tbody>
                            {!data?.length ? <tr><td colSpan={6} className='text-center'>giỏ hàng trống</td></tr> :
                                data.map((item: ICart) => {
                                    ...
                                    return (
                                        <tr key={item.id}>
                                            ...
                                            <td className='text-black py-4'>
                                                <button onClick={() => decreaseQuantity(product?.id || 0)} className="mx-2.5">-</button>
                                                {quantity}
                                                <button onClick={() => increaseQuantity(product?.id || 0)} className="mx-2.5">+</button>
                                            </td>
                                            <td className='text-black py-4'>{Intl.NumberFormat("en-US", { maximumFractionDigits: 0 }).format(product?.price * quantity)}</td>
                                            <td className='text-black py-4'>
                                                <button onClick={() => {
                                                    ...
                                                    {confirm === product.id ? "Xác nhận" : <img src="/src/assets/ant-design_delete-filled.png" alt="" />}
                                                </button>
                                            </td>
                                        </tr>
                                    )
